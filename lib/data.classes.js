...
export class News {
  ...
  create_article(data) {
    ...
    const content = (info) => {
      let fmd = info.content

      const mdc = new showdown.Converter();
      mdc.setOption('tables', true);
      mdc.setOption('emoji', true);
      mdc.setOption('headerLevelStart', 2);
      mdc.setOption('tasklist', true);
      mdc.setOption('customizedHeaderId', true);
      mdc.setOption('simplifiedAutoLink', true);
      mdc.setOption('openLinksInNewWindow', true);
      mdc.setFlavor('github')

      const html = mdc.makeHtml(fmd);
      const content_section = document.createElement('div');
      content_section.id = 'content';
      content_section.innerHTML = html;

      this.dom.classList.add('news');
      this.dom.appendChild(content_section);
    }
    ...
    const more_news = () => {
      ...
      new News({get: 'random', dom: section, type: 'list', permalink: this.permalink});
