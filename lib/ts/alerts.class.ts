...
class Modal {
    ...
    public create(options) {
        ...
        for (let i = 0; i < options.buttons.length; i++) {
            ...
            let button = document.createElement('button');
                ...
                if (options.buttons[i].type != undefined) { button.classList.add(options.buttons[i].type); }
        ...
        for (let i = 0; i < elements.length; i++) { container.appendChild(elements[i]); }
    ...
    public show(options) {
        ...
        if (background.childNodes.length > 0) { this.destroy(); }
        ...
        if (options.quit_on) {setTimeout(() => this.hide(), options.quit_on)}
        if (options.destroy_on) {setTimeout(() => this.destroy(), options.destroy_on)}
    }
    ...
    public destroy() {
        ...
        for (let i = 0; i < buttons.length; i++) { buttons[i].removeEventListener('click', this.buttons[i].action); }
