...
class Select_list {
  ...
  #new_li(name, value, has_subcategory = true) {
    ...
    let li_text = document.createElement('div');
        li_text.innerHTML = name;
    li.appendChild(li_text);

    // Añade propiedades de subcategoría o de valor según el caso
    if (has_subcategory) {
      li.classList.add('subcategory');
      li_text.addEventListener('click', () => switch_category(li, li_text.textContent) );
    } else {
      li.classList.add('value');
      li_text.dataset.value = value;
      li_text.addEventListener('click', () => this.#select_option(li_text.dataset.value) );
  ...
  #show_coincidences(search_value) {
    ...
    for (let i = 0; i < childs.length; i++) {
      ...
      let option_value = correct_text(element.firstChild.textContent);
